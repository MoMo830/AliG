...
def process_image_logic(image_path, s, source_img_cache=None):
    """
...
def generate_gcode_list(matrix, h_px, w_px, l_step, x_st, offX, offY, gc):
    """
...
def assemble_gcode(body, header_custom, footer_custom, settings, metadata):
...
def calculate_offsets(selected_origin, real_w, real_h, custom_x=0, custom_y=0):
    offX, offY = 0, 0
...
def generate_pointing_gcode(offX, offY, power, pause_cmd=None, use_s_mode=True, e_num=0):
