...
class LaserGeneratorApp(ctk.CTk):
    def __init__(self):
        ...
        self.icon_path = os.path.join(current_dir, "..", "assets", "icone_alig.ico")
    ...
    def setup_ui(self):
        ...
        self.btn_gen = ctk.CTkButton(self.sidebar, text="SIMULATE", fg_color="#1f538d", hover_color="#2a6dbd", height=50, font=("Arial", 13, "bold"), command=self.generate_gcode)
    ...
    def _setup_tabs_content(self):
        ...
        # --- TAB 5: CALIBRATION ---
        t_cal = self.tabview.tab("Calibration")

        # Section 1: Latency 
        frame_lat = ctk.CTkFrame(t_cal, fg_color="#2b2b2b")
        frame_lat.pack(fill="x", padx=10, pady=10)

        ctk.CTkLabel(frame_lat, text="LATENCY TEST", font=("Arial", 12, "bold")).pack(pady=(10, 5))
        
        ctk.CTkLabel(frame_lat, 
                     text="Generates a black square to check\nif your latency setting is correctly set.\n(caution : Max power parameter is used)", 
                     font=("Arial", 10), 
                     text_color="#aaaaaa").pack(pady=2)

        ctk.CTkButton(frame_lat, 
                      text="GENERATE/LOAD LATENCY TEST", 
                      fg_color="#d35400", 
                      hover_color="#e67e22",
                      command=self.generate_latency_image).pack(fill="x", padx=20, pady=10)
        
        # Info sur le fichier (Vert)
        ctk.CTkLabel(frame_lat, 
                     text="Saves 'latency_test.png' in program folder", 
                     font=("Arial", 9, "italic"), 
                     text_color="#27ae60").pack(pady=(0, 10))

        # Section 2: Power Scale (1x10)
        frame_grid = ctk.CTkFrame(t_cal, fg_color="#2b2b2b")
        frame_grid.pack(fill="x", padx=10, pady=10)

        ctk.CTkLabel(frame_grid, text="POWER SCALE TEST", font=("Arial", 12, "bold")).pack(pady=(10, 5))
        
        ctk.CTkLabel(frame_grid, 
                     text="Generates 10 squares from min to max power.", 
                     font=("Arial", 10), 
                     text_color="#aaaaaa").pack(pady=2)

        ctk.CTkButton(frame_grid, 
                      text="GENERATE/LOAD POWER SCALE", 
                      fg_color="#d35400", 
                      hover_color="#e67e22",
                      command=self.generate_test_grid).pack(fill="x", padx=20, pady=10)

        # Info sur le fichier (Vert)
        ctk.CTkLabel(frame_grid, 
                     text="Saves 'power_scale.png' in program folder", 
                     font=("Arial", 9, "italic"), 
                     text_color="#27ae60").pack(pady=(0, 10))
    ...
    def generate_latency_image(self):
        """ Génère un rectangle noir pur de 50x250px pour tester la latence """
        # Dimensions demandées
        width, height = 30, 30
        # Création d'une image entièrement noire (0 = noir en mode 'L')
    ...
    def generate_test_grid(self):
        """ Génère une image de 500x50px avec 10 carrés séparés """
        block_size = 50
        num_steps = 10
        padding_x = 5  # Espace seulement entre les carrés
        ...
        for i in range(num_steps):
            # i=0 (gauche) -> 255 (blanc) | i=9 (droite) -> 0 (noir)
            gray_level = int(255 - (i * (255 / (num_steps - 1))))
            
            x_start = (i * block_size) + padding_x
            x_end = ((i + 1) * block_size) - padding_x
            
            # y_start=0 et y_end=height pour supprimer les blancs en haut et en bas
        ...
        self.input_image_path = temp_path
        # Astuce : On remet Gamma et Thermal à 1.0 pour que le test soit linéaire
        if "gamma" in self.controls: self.controls["gamma"]["slider"].set(1.0)
        if "thermal" in self.controls: self.controls["thermal"]["slider"].set(1.0)
        # On force l'update des champs texte liés
        self.sync_from_slider(self.controls["gamma"]["slider"], self.controls["gamma"]["entry"], 1.0, False, 2)
        self.sync_from_slider(self.controls["thermal"]["slider"], self.controls["thermal"]["entry"], 1.0, False, 2)
    ...
    def process_logic(self):
      ...
      settings = {
        ...
        "contrast": self.get_val(self.controls["contrast"]),  # <-- Vérifiez bien cette ligne
      ...
      # 2. GESTION DU CACHE (Ici on a le droit d'utiliser 'self')
      # On vérifie si on doit vider le cache car l'image a changé
      current_cache = None
    ...
    def generate_gcode(self):
        # --- 0. CAPTURE SÉCURISÉE DES VRAIS WIDGETS ---
        try:
            # On récupère les états des switches (BooleanVar)
            is_pointing = self.origin_pointer_var.get()
            is_framing = self.frame_var.get()
            
            # On récupère les valeurs des entrées
            f_pwr_raw = self.frame_power_entry.get()
            # On nettoie le menu (ex: "20%" -> "20" -> 0.20)
            f_ratio_str = self.frame_feed_ratio_menu.get().replace('%', '')
            f_pause = self.pause_cmd_entry.get().strip()
            ...
            print(f"Erreur d'accès aux widgets : {e}")
            return

        # --- 1. INDICATEUR VISUEL ---
        original_text = self.btn_gen.cget("text")
        self.btn_gen.configure(text="GENERATING... ", state="disabled", fg_color="#d35400")
        self.update_idletasks() 

        try:
            # --- 2. CALCULS DES DIMENSIONS ---
            ...
            # --- 3. PRÉPARATION DU CADRAGE ET POINTING ---
            framing_gcode = ""
            ...
            if is_pointing or is_framing:
                try:
                    pwr = float(f_pwr_raw)
                    ratio = float(f_ratio_str) / 100.0
                    base_feed = self.get_val(self.controls["feedrate"])
                    f_feed = int(base_feed * ratio)
                except:
                    pwr, ratio, f_feed = 0.0, 0.2, 600

                # A. Appel du POINTING (Engine)
                if is_pointing:
                    framing_gcode += generate_pointing_gcode(
                        offX, offY, pwr, 
                        pause_cmd=f_pause if f_pause else None,
                        use_s_mode=use_s_mode, 
                        e_num=e_num
                    )
                    framing_gcode += "\n"

                # B. Appel du FRAMING (Engine)
                if is_framing:
                    framing_gcode += generate_framing_gcode(
            ...
            # --- 4. PRÉPARATION DES DONNÉES POUR LA SIMU ---
            pre = self.get_val(self.controls["premove"]) 
            user_min_p = self.get_val(self.controls["min_p"]) 
            user_max_p = self.get_val(self.controls["max_p"])
            ...
            stats = {
               ...
               "pixel_size_x": x_st,  
               "pixel_size_y": l_step,
               "min_power": user_min_p,
               "max_power": user_max_p,
               "real_w": real_w,
               "real_h": real_h,
               "est_sec": int(est_min * 60),
               "file_name": os.path.basename(self.input_image_path).split('.')[0] + ".nc",
               "is_pointing": is_pointing,
               "is_framing": is_framing
            }

            # --- 5. APPEL DE LA SIMULATION ---
            # On passe 'pre' via premove_val
            sim_win = SimulationWindow(self, stats, matrix, premove_val=pre, framing_data=framing_gcode)
            sim_win.grab_set()          
            self.wait_window(sim_win)

            if not sim_win.confirmed:
                self.btn_gen.configure(text=original_text, state="normal", fg_color="#1f538d")
                return

            # --- 6. GÉNÉRATION ET ASSEMBLAGE FINAL ---
            ...
            use_s_mode = "S (Spindle)" in current_cmd_mode
            user_firing_on = current_firing_mode.split('/')[0]

            gc_settings = {
                'e_num': e_num, 'use_s_mode': use_s_mode,
                'ratio': ctrl_max / 100.0, 'ctrl_max': ctrl_max,
                'premove': pre, 'feedrate': f_int,
                'offset_latence': (f_int * m67_ms) / 60000 
            }

            gcode_body = generate_gcode_list(matrix, h_px, w_px, l_step, x_st, offX, offY, gc_settings)

            metadata = {
                'version': self.version,
                'mode': current_cmd_mode.split(' ')[0],
                'firing_cmd': user_firing_on,
                'gray_steps': int(self.get_val(self.controls["gray_steps"])),
                'framing_code': framing_gcode
            }

            final_content = assemble_gcode(
                gcode_body, 
                self.txt_header.get("1.0", "end-1c").strip(),
                self.txt_footer.get("1.0", "end-1c").strip(),
                gc_settings,
                metadata
            )

            # --- 7. ÉCRITURE ---
            full_path = os.path.join(self.output_dir, stats["file_name"])
    ...
    def get_all_settings_data(self):
        ...
        # Note : On peut aussi inclure la géométrie de la fenêtre ici
