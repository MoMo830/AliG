...
class RasterViewQt(QWidget):
    ...
    def _build_sidebar(self):
        ...
        self.btn_gen = QPushButton(self.t.get("simulate_gcode", "Simulate G-Code"))
    ...
    def _make_file_buttons(self):
        ...
        self.btn_output = QPushButton(self.t.get("select_output", "Select Output"))
    ...
    def _setup_tab_geom(self):
        ...
        lbl_mode = QLabel(self.t.get("raster_mode", "raster_mode"))
        ...
        self._add_combo(lo, "origin_point", origin_opts, "origin_mode",
        ...
        self._add_simple_input(cf_lo, "custom_offset_x", 0.0, "custom_x")
        self._add_simple_input(cf_lo, "custom_offset_y", 0.0, "custom_y")
        lo.addWidget(self.custom_offset_frame)

    def _setup_tab_img(self):
        lo = self._tab_img._inner
        self._add_slider_input(lo, "contrast", -1.0, 1.0, 0.0, "contrast")
        self._add_slider_input(lo, "gamma",     0.1, 6.0, 1.0, "gamma")
        self._add_slider_input(lo, "thermal",   0.1, 3.0, 1.5, "thermal")

        sw_row = QHBoxLayout()
        lbl_inv = QLabel(self.t.get("invert_color", "invert_color"))
    ...
    def _setup_tab_laser(self):
        ...
        self._add_slider_input(lo, "feedrate", 500, 20000, 3000, "feedrate", is_int=True)
        self._add_slider_input(lo, "overscan",    0,   50,    10.0, "premove")
        ...
        self._add_slider_input(lo, "laser_latency", -20, 20, 0, "laser_latency")
        self._add_slider_input(lo, "gray_steps", 2, 256, 256, "gray_steps", is_int=True)
    ...
    def _setup_tab_gcode(self):
        ...
        hdr_row = QHBoxLayout()
        lbl_glob = QLabel(self.t.get("global_machine_params", "Global Machine Params"))
        lbl_glob.setStyleSheet("color:#FF9500;font-weight:bold;font-size:11px;")
        self.translation_map[lbl_glob] = "global_machine_params"
        ...
        self._add_combo(mc_lo, "cmd_mode",
                        ["M67 (Analog)", "S (Spindle)"], "cmd_mode")
        row_em = QHBoxLayout()
        self._add_simple_input(row_em, "m67_output", 0, "m67_e_num", precision=0, box_length=40)
        self._add_simple_input(row_em, "ctrl_max_value", 100, "ctrl_max", precision=0)
        mc_lo.addLayout(row_em)
        self._add_combo(mc_lo, "firing_mode",
        ...
        lbl_h = QLabel(self.t.get("gcode_header", "gcode_header"))
        ...
        lbl_f = QLabel(self.t.get("gcode_footer", "gcode_footer"))
        ...
        lbl_fr = QLabel(self.t.get("point_fram_options", "Pointing / Framing Options"))
        lbl_fr.setStyleSheet("font-weight:bold;font-size:11px;color:#ddd;")
        self.translation_map[lbl_fr] = "point_fram_options"
        lo.addWidget(lbl_fr)
        ...
        lbl_ptr = QLabel(self.t.get("origin_pointing", "origin_pointing"))
        ...
        lbl_frm = QLabel(self.t.get("framing_option", "framing_option"))
    ...
    def _add_simple_input(self, layout, label_key, default, key, precision=2, box_length=80):
        row = QHBoxLayout()
        lbl = QLabel(self.t.get(label_key, label_key))
        lbl.setStyleSheet("color:#ddd;font-size:12px;")
        self.translation_map[lbl] = label_key
    ...
    def _add_combo(self, layout, label_key, options, key, callback=None):
        lbl = QLabel(self.t.get(label_key, label_key))
        lbl.setStyleSheet("color:#ddd;font-size:12px;")
        self.translation_map[lbl] = label_key
        layout.addWidget(lbl)
    ...
    def _seg_btn_style(self):
        ...
        return (
            f"QPushButton{{background:{c['seg_bg']};color:{c['seg_text']};"
            f"border:1px solid {c['seg_border']};border-radius:4px;"
            f"padding:2px 8px;min-width:60px;font-size:11px;}}"
            f"QPushButton:checked{{background:#1F6AA5;color:white;border-color:#2a6dbd;}}"
            f"QPushButton:hover:!checked{{background:{c['seg_hover']};color:{c['seg_active_text']};}}"
        )

    def _entry_style(self):
        c = self._theme_colors
        return (f"QLineEdit{{background:{c['bg_entry']};border:1px solid {c['border_strong']};"
                f"border-radius:4px;color:{c['text']};padding:2px;}}")

    def _textedit_style(self):
        c = self._theme_colors
        return (f"QTextEdit{{background:{c['bg_entry_alt']};border:1px solid {c['border_strong']};"
    ...
    def _combo_style(self):
        ...
        return (
            f"QComboBox{{background:{c['bg_entry']};border:1px solid {c['border_strong']};"
            ...
            f"QComboBox QAbstractItemView{{background:{c['bg_entry']};color:{c['text']};"
    ...
    def _on_raster_change(self, key):
        ...
        for k, b in self._btn_raster.items():
            b.setChecked(k == key)
        if key == "vertical":
            self.width_label_widget.setText(self.t.get("target_height", "target_height"))
            self.lbl_force_width.setText(self.t.get("force_height", "force_height"))
        else:
            self.width_label_widget.setText(self.t.get("target_width", "target_width"))
            self.lbl_force_width.setText(self.t.get("force_width", "force_width"))
    ...
    def apply_lock_state(self):
        ...
        self.lock_btn.setStyleSheet(
            f"QPushButton{{background:{style};color:{c['locked_text']};"
    ...
    def apply_theme(self, colors):
        ...
        bg_tabs   = colors['bg_tabs']
        bg_tab    = colors['bg_tab']
        bg_right  = colors['bg_deep']
        bg_stats  = colors['bg_stats']
        bg_prev   = colors['bg_preview']
        col_prev  = colors['preview_text']
        btn_prof  = colors['profile_btn']
        btn_prof_text = colors['profile_btn_text']
        sec_brd   = colors['border_light']
        stats_col = colors['stats_text']
        canvas_fc = bg_right
        ax_col    = colors['stats_text']
        spine_col = colors['border']
        ph_col    = colors['seg_text']
        entry_col = colors['text']
        ...
        if hasattr(self, "_line_step_value_lbl"):
            self._line_step_value_lbl.setStyleSheet(
                ...
                f"background:{bg_stats};border:1px solid {border};"
                "border-radius:4px;padding:2px 8px;"
            )
        ...
        if hasattr(self, "btn_reset_view"):
            self.btn_reset_view.setStyleSheet(
                f"QPushButton{{background:{colors['btn_neutral']};color:{text_sec};border:none;"
                f"border-radius:4px;font-size:10px;}}"
                f"QPushButton:hover{{background:{colors['btn_neutral_hover']};color:{entry_col};}}"
            )
            from gui.utils_qt import get_svg_pixmap
    ...
    def update_texts(self):
        ...
        # Combo origin_mode — les items sont "(key, texte)" fixés à la création
        origin_ctrl = self.controls.get("origin_mode")
        if origin_ctrl:
            combo = origin_ctrl["combo"]
            ...
            for i, key in enumerate(ORIGIN_KEYS):
                if i < combo.count():
                    combo.setItemText(i, self.t_orig.get(key, key))
            combo.blockSignals(False)
            # Restaurer la sélection par userData
            if current_data:
                for i in range(combo.count()):
